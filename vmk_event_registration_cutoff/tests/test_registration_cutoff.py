...
class TestRegistrationCutoff(TransactionCase):
    ...
    def test_the_global_cutoff_applies(self):
    ...
    def test_an_event_may_override_the_global_cutoff(self):
        self._set_default(24)
        event = self._event(starts_in_hours=12, vmk_cutoff_custom=True, vmk_cutoff_hours=2)
    ...
    def test_an_override_of_zero_closes_at_the_start(self):
        ...
        self.assertTrue(event.event_registrations_open)
        past = self._event(starts_in_hours=-1, vmk_cutoff_custom=True, vmk_cutoff_hours=0)
    ...
    def test_a_slot_is_retired_by_the_cutoff_not_just_its_start(self):
        self._set_default(24)
        event = self._event(starts_in_hours=-1, length_hours=24 * 10, is_multi_slots=True)
        ...
        self.assertNotIn(soon, open_slots, "within the 24 hour cutoff")
